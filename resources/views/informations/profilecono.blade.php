...
@section('content')
    <div class="container">
        ...
        <p><strong>Nom: </strong> {{ $economiste->nom ?? 'Non renseigné' }}</p>
        <p><strong>Prénom: </strong> {{ $economiste->prenom ?? 'Non renseigné' }}</p>
        <p><strong>Sexe: </strong> {{ $economiste->sexe ?? 'Non renseigné' }}</p>
        <p><strong>Date de Naissance: </strong> {{ $economiste->datenaissance ?? 'Non renseigné' }}</p>
        <p><strong>Titre: </strong> {{ $economiste->titre ?? 'Non renseigné' }}</p>
        <p><strong>Date de Nomination: </strong> {{ $economiste->datenomin ?? 'Non renseigné' }}</p>
        <p><strong>Numéro de Téléphone: </strong> {{ $economiste->telephone ?? 'Non renseigné' }}</p>
        <p><strong>Email: </strong> {{ $economiste->email ?? 'Non renseigné' }}</p>
        <p><strong>Expertise: </strong> {{ $economiste->expertise ?? 'Non renseigné' }}</p>
        <p><strong>Collège: </strong> {{ $economiste->college ?? 'Non renseigné' }}</p>
        <p><strong>Spécialité: </strong> {{ $economiste->specialite ?? 'Non renseigné' }}</p>
        <p><strong>Travaux Significatifs: </strong>{{ $economiste->travauxSign }}</p>
        <p><strong>Implication Communautaire: </strong>{{ $economiste->communautaire }}</p>
        ...
        @foreach ($economiste->parrains as $parrain)
        ...
        <h2>Diplômes</h2>
        @foreach ($economiste->diplomes as $diplome)
            <div>
        ...
        @foreach ($economiste->experiences as $experience)
            <div>
        ...
        @foreach ($economiste->responsabilites as $responsabilite)
            <div>
        ...
        <h2>Ouvrages</h2>
        @foreach ($economiste->ouvrages as $ouvrage)
            <div>
        ...
        <h2>Articles</h2>
        @foreach ($economiste->articles as $article)
            <div>
        ...
        <H2>Brevets</H2>
        @foreach ($economiste->brevets as $brevet)
            <div>
        ...
        <h2>Commissions</h2>
        @foreach ($economiste->commissions as $comm)
        ...
        <h2>Distinctions</h2>
        @foreach ($economiste->distinctions as $distinction)
        ...
        @foreach ($economiste->preuveChercheurs as $file)
            <?php /*<p><strong>Type:</strong> * {{ $file['type']}} </p>*/?>


            <p><strong>Nom Original:</strong> {{ $file['nom_originale'] }}</p>

            <p><strong>Chemin:</strong> <a target="_blank" href="{{ url($file['path'])}}">Voir le fichier</a>
            </p>
        @endforeach
        ...
        <p><strong>Contribution majeure dans les domaines du collège postulé :</strong>
            {{ $economiste->contribution ?? '' }}</p>

        <hr>

        <h2>Apport Particulier à l'academie</h2>
        <p><strong> {{ $economiste->apport ?? 'Non renseigné' }} </strong>
            </p>
