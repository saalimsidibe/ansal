...
@section('content')
    <div class="container">
        ...
        <p><strong>Nom: </strong> {{ $medecin->nom ?? 'Non renseigné' }}</p>
        <p><strong>Prénom: </strong> {{ $medecin->prenom ?? 'Non renseigné' }}</p>
        <p><strong>Sexe: </strong> {{ $medecin->sexe ?? 'Non renseigné' }}</p>
        <p><strong>Date de Naissance: </strong> {{ $medecin->datenaissance ?? 'Non renseigné' }}</p>
        <p><strong>Titre: </strong> {{ $medecin->titre ?? 'Non renseigné' }}</p>
        <p><strong>Date de Nomination: </strong> {{ $medecin->datenomin ?? 'Non renseigné' }}</p>
        <p><strong>Numéro de Téléphone: </strong> {{ $medecin->telephone ?? 'Non renseigné' }}</p>
        <p><strong>Email: </strong> {{ $medecin->email ?? 'Non renseigné' }}</p>
        <p><strong>Expertise: </strong> {{ $medecin->expertise ?? 'Non renseigné' }}</p>
        <p><strong>Collège: </strong> {{ $medecin->college ?? 'Non renseigné' }}</p>
        <p><strong>Spécialité: </strong> {{ $medecin->specialite ?? 'Non renseigné' }}</p>
        <p><strong>Travaux Significatifs: </strong>{{ $medecin->travauxSign }}</p>
        <p><strong>Implication Communautaire: </strong>{{ $medecin->communautaire }}</p>
        ...
        @foreach ($medecin->parrains as $parrain)
        ...
        <h2>Diplômes</h2>
        @foreach ($medecin->diplomes as $diplome)
            <div>
        ...
        @foreach ($medecin->experiences as $experience)
            <div>
        ...
        @foreach ($medecin->responsabilites as $responsabilite)
            <div>
        ...
        <h2>Ouvrages</h2>
        @foreach ($medecin->ouvrages as $ouvrage)
            <div>
        ...
        <h2>Articles</h2>
        @foreach ($medecin->articles as $article)
            <div>
        ...
        <H2>Brevets</H2>
        @foreach ($medecin->brevets as $brevet)
            <div>
        ...
        <h2>Commissions</h2>
        @foreach ($medecin->commissions as $comm)
        ...
        <h2>Distinctions</h2>
        @foreach ($medecin->distinctions as $distinction)
        ...
        @foreach ($medecin->preuveChercheurs as $file)
        ...
        <h2>Contribution</h2>
        <p><strong>Contribution majeure dans les domaines du collège postulé :</strong> {{ $medecin->contribution ?? '' }}
        </p>
