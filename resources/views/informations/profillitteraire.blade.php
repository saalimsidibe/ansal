...
<h1>Résumé des Informations</h1>
    ...
    <p><strong>Nom: </strong> {{ $litteraire->nom ?? 'Non renseigné' }}</p>
    <p><strong>Prénom: </strong> {{ $litteraire->prenom ?? 'Non renseigné' }}</p>
    <p><strong>Sexe: </strong> {{$litteraire->sexe  ?? 'Non renseigné' }}</p>
    <p><strong>Date de Naissance: </strong> {{$litteraire->datenaissance ?? 'Non renseigné'}}</p>
    <p><strong>Titre: </strong> {{ $litteraire->titre ?? 'Non renseigné' }}</p>
    <p><strong>Date de Nomination: </strong> {{$litteraire->datenomin ?? 'Non renseigné'}}</p>
    <p><strong>Numéro de Téléphone: </strong> {{$litteraire->telephone  ?? 'Non renseigné' }}</p>
    <p><strong>Email: </strong> {{$litteraire->email ?? 'Non renseigné' }}</p>
    <p><strong>Expertise: </strong> {{ $litteraire->expertise ?? 'Non renseigné' }}</p>
    <p><strong>Collège: </strong> {{ $litteraire->college ?? 'Non renseigné' }}</p>
    <p><strong>Spécialité: </strong> {{ $litteraire->specialite ?? 'Non renseigné' }}</p>
    <p><strong>Travaux Significatifs: </strong>{{ $litteraire->travauxSign}}</p>
    <p><strong>Implication Communautaire: </strong>{{ $litteraire->communautaire}}</p>
    ...
    @foreach($litteraire->parrains as $parrain)
    ...
    <h2>Diplômes</h2>
    @foreach ($litteraire->diplomes as $diplome)
        <div>
    ...
    @foreach ($litteraire->experiences as $experience)
        <div>
    ...
    @foreach ($litteraire->responsabilites as $responsabilite)
        <div>
    ...
    <h2>Ouvrages</h2>
    @foreach ($litteraire->ouvrages as $ouvrage)
        <div>
    ...
    @endforeach
         ...
         <h2>Articles</h2>
    @foreach ($litteraire->articles as $article)
        <div>
    ...
    <H2>Brevets</H2>
    @foreach ($litteraire->brevets as $brevet)
        <div>
    ...
    @endforeach
        ...
        <h2>Commissions</h2>
        @foreach ($litteraire->commissions as $comm)
        ...
        @endforeach
         ...
         <h2>Distinctions</h2>
    @foreach ($litteraire->distinctions as $distinction)
    ...
    <h2>Contribution</h2>
    <p><strong>Contribution majeure dans les domaines du  collège postulé  :</strong> {{$litteraire->contribution ??'' }}</p>

    <hr>

    <h2>Apport Particulier à l'academie</h2>
    <p><strong></strong><{{$litteraire->apport ?? 'Non renseigné' }}/p>
</body>
